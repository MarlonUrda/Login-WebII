...
const languages = [ //Ejemplo que venía por defecto en el buscador de usuarios
...
const roles = [ //Ejemplo para opciones no implementado
  { value: '1', label: 'Ingeniero Software' },
  { value: '2', label: 'Desarrollador' },
  { value: '3', label: 'Programador' },
  { value: '4', label: 'Documentista' },
  { value: '5', label: 'Recurso' },
]
</script>
...
<template>
    ...
    ><UserRoundPlus class="mt-[2%] mr-2 h-4 w-4 inline"/>Añadir Integrantes</Button>
        </SheetTrigger>
  ...
  <SheetHeader>
    ...
    <SheetDescription class="text-center mb-4">
      Desde aquí puedes añadir nuevos integrantes a este proyecto, así como cambiar su rol o eliminarlos.
    </SheetDescription>
  </SheetHeader>
  <form class="space-y-6 text-center" @submit="onSubmit">
          ...
          <PopoverTrigger as-child>
            <FormControl>
              <Button
                variant="outline"
                role="combobox"
                :class="cn('w-[200px] justify-between ml-[50%] -translate-x-1/2 mt-1 -mb-3', !values.language && 'text-muted-foreground')"
              >
    ...
    <Button type="submit"><Plus class="mt-[2%] mr-1 -ml-1 h-4 w-4 inline" />
      Añadir al Proyecto
    </Button>
  </form>
  <ScrollArea class="w-[85%] mt-[1%] mb-[1%] -translate-x-1/2 left-1/2">
    <Table>
      <TableHeader class="text-lg italic">
        <TableRow>
          ...
          <TableHead class="text-center font-bold">Nombre de Usuario</TableHead>
          <TableHead class="text-center font-bold">Nombre y Apellido</TableHead>
      ...
      <TableBody class="text-base">
        ...
        <TableCell class="text-center italic">{{ Member.User }}</TableCell>
    ...
    <PopoverTrigger as-child>
      ...
      >
        {{ value ? role.find((framework) => framework.value === value)?.label : 'Elegir rol a asignar...' }}
  ...
  <SheetFooter>
    <Sheet>
            <SheetTrigger as-child>
                <Button type="submit" class="left-[1.5%] absolute"><Contact class="mt-[2%] mr-2 h-4 w-4 inline" />Añadir Invitado</Button>
            </SheetTrigger>
      ...
      <div class="grid gap-4 py-4">
        <div class="grid grid-cols-4 items-center gap-4">
          <Label for="cedula" class="text-center">
            N°. de Cédula
          </Label>
          <Input id="cedula" placeholder="Ej.: 12345678" class="col-span-3" />
        </div>
        <div class="grid grid-cols-4 items-center gap-4">
          <Label for="name" class="text-center">
            Nombre y Apellido
          </Label>
          <Input id="name" placeholder="Ej.: Jubert Pérez" class="col-span-3" />
        </div>
      </div>
      <SheetFooter>
        <SheetClose as-child>
    ...
    <SheetClose as-child>
        <Button type="submit" class="bg-blue-500"><Check class="mt-[2%] mr-2 h-4 w-4 inline" />Guardar Cambios</Button>
    </SheetClose>
  </SheetFooter>
</SheetContent>
    </Sheet>
