...
class DbQueries {
    ...
    async getUsersByPassword(password) {
        const result = await this.pool.query("SELECT * FROM users WHERE password = $1", [password]);
        return result;
    }
    ...
    async updateResetPasswordToken(email, token, expires) {
        try {
            const result=await this.pool.query('UPDATE users SET resetPasswordToken = $1, resetPasswordExpires = $2 WHERE email = $3', [token, expires, email]);
            if(1!=result.rowCount){
                console.log('Error al actualizar el token de restablecimiento de contraseña');
                return false;
            }
            return true;
    ...
    async getUserwithValidToken(token) {
        try {
            ...
            console.error("Error al Buscar en la base de datos el token de restablecimiento de contraseña:", error);
